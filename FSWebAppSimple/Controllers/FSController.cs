...
namespace FSWebAppSimple.Controllers
{
    ...
    public class FSController : ApiController
    {
        ...
        {
            //processing request in one app, one thread--------------------------------------------------------------------------------------------------------
            FinancialStatement fs = await ProcessFinancialStatement(fsArg.Firstname, fsArg.Lastname, fsArg.Income, fsArg.Age, fsArg.Photo);
            if (fs == null)
                return ResponseMessage(Request.CreateErrorResponse(HttpStatusCode.InternalServerError, "Something went wrong with images generator"));
            return Ok();
        }

        public async Task<FinancialStatement> ProcessFinancialStatement(string firstname, string lastname, int income, int age, byte[] photo)
        {
